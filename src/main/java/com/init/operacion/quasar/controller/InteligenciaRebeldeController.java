...
import com.init.operacion.quasar.model.PosicionSatelite;
...
public class InteligenciaRebeldeController {
	...
	public ResponseEntity topSecret(RequestEntity<PosicionSatelite> requestEntitySatelite) {
		...
		try {
			return ResponseEntity.status(HttpStatus.OK)
					.body(intelligenceService.ObtenerNaveEspacial(requestEntitySatelite));
	...
	public ResponseEntity topsecret_split(@PathVariable("name") String name,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.OK)
					.body(intelligenceService2.ObtenerNaveEspacialSplit(requestEntitySatelite, name));
		} catch (MensajeExcepcion e) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, e.getMessage(), e);
		}
	}
